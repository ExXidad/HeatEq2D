...
bool BoundingRect::innerShellContains(const double &h, const double &x, const double &y)
{
    return (
            ((size[0][0] <= x && x <= size[0][1]) && (size[1][0] <= y && y <= size[1][1]))
            &&
            !((size[0][0] + h <= x && x <= size[0][1] - h) && (size[1][0] + h <= y && y <= size[1][1] - h))
    );
}

bool BoundingRect::outerShellContains(const double &h, const double &x, const double &y)
{
    return (
            !((size[0][0] <= x && x <= size[0][1]) && (size[1][0] <= y && y <= size[1][1]))
            &&
